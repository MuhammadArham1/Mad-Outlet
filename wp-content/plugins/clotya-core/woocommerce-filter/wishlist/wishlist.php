...
if ( ! class_exists( 'KlbWishlist' ) ) {
	class KlbWishlist {
		...
		function init() {
			...
			// add button for clotya_wishlist_action
			add_action( 'clotya_wishlist_action', [ $this, 'clotya_wishlist_button' ], 11 );
		}
		...
		function wishlist_add() {
			...
			if ( ( $product_id = (int) sanitize_text_field( $_POST['product_id'] ) ) > 0 ) {
				...
				if ( ! array_key_exists( $product_id, $products ) ) {
					...
					$return['notice'] = esc_html__( '{name} has been added to Wishlist.', 'clotya-core' );
				} else {
					$return['notice'] = esc_html__( '{name} is already in the Wishlist.', 'clotya-core' );
				}
				...
				$return['totalcount'] = sprintf(_n('%d person', '%d people', $totalcount, 'clotya-core'), $totalcount);
				$return['data']   = [
					'key'       => self::get_key(),
					'ids'       => self::get_ids(),
				];
				...
				$product_id       = 0;
				$return['status'] = 0;
				$return['notice'] = esc_html__( 'Have an error, please try again!', 'clotya-core' );
			}
		...
		function wishlist_remove() {
			...
			if ( ( $product_id = (int) sanitize_text_field( $_POST['product_id'] ) ) > 0 ) {
				...
				if ( array_key_exists( $product_id, $products ) ) {
					...
					$return['count']  = count( $products );
					$return['status'] = 1;
					$return['notice'] = esc_html__( 'Product has been removed from the Wishlist.', 'clotya-core' );
					$return['data']   = [
						'key'       => self::get_key(),
						'ids'       => self::get_ids(),
					];
					...
					if ( empty( $products ) ) {
						$return['content'] = self::wishlist_content( $key, esc_html__( 'There are no products on the Wishlist!', 'clotya-core' )) . '</div>';
					}
				} else {
					$return['notice'] = esc_html__( 'The product does not exist on the Wishlist!', 'clotya-core' );
				}
			} else {
				$product_id       = 0;
				$return['notice'] = esc_html__( 'Have an error, please try again!', 'clotya-core' );
		...
		function clotya_wishlist_button() {
			...
			if($wishlist == '1'){
				...
				if ( $attr_id ) {
					...
					if ( array_key_exists( $attr_id, self::$products ) || in_array( $attr_id, array_column( self::$products, 'parent' ) ) ) {
						...
						$text  = apply_filters( 'klbwl_button_text_added', esc_html__( 'View wishlist', 'clotya-core' ) );
					} else {
						$text = apply_filters( 'klbwl_button_text', esc_html__( 'Add to wishlist', 'clotya-core' ) );
					}
		...
		function clotya_klbwl_total_count(){
			global $product;
			$output;
		...
		function account_items( $items ) {
			if ( ! isset( $items['wishlist'] ) ) {
				$items['wishlist'] = apply_filters( 'klbwl_myaccount_wishlist_label', esc_html__( 'Wishlist', 'clotya-core' ) );
			}
		...
		function enqueue_scripts() {
			...
			// main style
			wp_enqueue_style( 'clotya-wishlist', plugins_url( 'css/wishlist.css', __FILE__ ), false, '1.0');

			// main js
			wp_enqueue_script( 'clotya-wishlist',  plugins_url( 'js/wishlist.js', __FILE__ ),[
				'jquery',
				'js-cookie'
			], true );
			...
			// localize
			wp_localize_script( 'clotya-wishlist', 'klbwl_vars', [
					'ajax_url'            => admin_url( 'admin-ajax.php' ),
					'wishlist_url'        => self::get_url(),
					'empty_confirm'       => esc_html__( 'This action cannot be undone. Are you sure?', 'clotya-core' ),
					'delete_confirm'      => esc_html__( 'This action cannot be undone. Are you sure?', 'clotya-core' ),
					'menu_text'           => apply_filters( 'klbwl_menu_item_label', esc_html__( 'Wishlist', 'clotya-core' ) ),
					'button_text'         => apply_filters( 'klbwl_button_text', esc_html__( 'Add to wishlist', 'clotya-core' ) ),
					'button_text_added'   => apply_filters( 'klbwl_button_text_added', esc_html__( 'View wishlist', 'clotya-core' ) ),
				]
			);
		}
		...
		function get_items( $key, $layout = null ) {
			...
			if ( is_array( $products ) && ( count( $products ) > 0 ) ) {
				...
				echo '<thead>';
				echo '<tr>';
				echo '<th class="product-thumbnail"><span class="screen-reader-text">'.esc_html__( 'Thumbnail image', 'clotya-core' ).'</span></th>';
				echo '<th class="product-name">'.esc_html__( 'Product', 'clotya-core' ).'</th>';
				echo '<th class="product-price">'.esc_html__( 'Price', 'clotya-core' ).'</th>';
				echo '<th class="product-date">'.esc_html__( 'Date Added', 'clotya-core' ).'</th>';
				echo '<th class="product-subtotal">'.esc_html__( 'Stock', 'clotya-core' ).'</th>';
				echo '<th class="product-addtocart">'.esc_html__( 'Add to cart', 'clotya-core' ).'</th>';
				echo '<th class="product-remove"><span class="screen-reader-text">'.esc_html__( 'Remove item', 'clotya-core' ).'</span></th>';
				echo '</tr>';
				echo '</thead>';
				echo '<tbody>';
				...
				echo '<div class="klbwl-no-result">
							<div class="cart-empty-page">
								...
								<p class="cart-empty">'.esc_html__( 'The wishlist table is empty.', 'clotya-core' ).'</p>
								<p class="return-to-shop">
									<a class="button wc-backward'.esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ).'" href="'.esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ).'">
										'.esc_html( apply_filters( 'woocommerce_return_to_shop_text', esc_html__( 'Return to shop', 'clotya-core' ) ) ).'
									</a>
								</p>
							</div>
		...
		function wishlist_content( $key = false, $message = '' ) {
			...
			$name     = esc_html__( 'Wishlist', 'clotya-core' );
			...
			<div class="klbwl-popup-inner" data-key="<?php echo esc_attr( $key ); ?>">
				<div class="klbwl-popup-content">
					...
					<a class="btn primary klbwl-popup-close" href="#" data-url="#">
						<i class="klbth-icon-cancel"></i> <?php echo esc_html__( 'Close', 'clotya-core' ); ?>
					</a>
				</div>
			</div>
			<?php
		...
		public static function get_page_id() {
			if(get_theme_mod('clotya_wishlist_page')){
				return absint(get_theme_mod('clotya_wishlist_page'));
			}
		...
		function clotya_delete_all_data(){
