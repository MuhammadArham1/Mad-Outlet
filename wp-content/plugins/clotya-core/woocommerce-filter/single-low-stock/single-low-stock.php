...
## Scripts
*************************************************/
function clotya_single_low_stock_scripts() {
	wp_enqueue_style( 'klb-single-low-stock',   plugins_url( 'css/single-low-stock.css', __FILE__ ), false, '1.0');
}
add_action( 'wp_enqueue_scripts', 'clotya_single_low_stock_scripts' );

/*************************************************
## Product Low Stock
*************************************************/
function clotya_product_low_stock() {
	
	global $product;
	
	$managestock = $product->managing_stock();
	$stock_quantity = $product->get_stock_quantity();
	$stock_format = esc_html__('This item is low in stock.','clotya');
	$stock_format2 = esc_html__('Item(s) left: %s','clotya');
	$stock_poor = '';
	$low_stock_amount = wc_get_low_stock_amount($product);
	
	if($managestock && $stock_quantity < $low_stock_amount) {
		$stock_poor .= '<div class="klb-product-low-stock">';
...
add_action( 'woocommerce_after_add_to_cart_button',  'clotya_product_low_stock' );
