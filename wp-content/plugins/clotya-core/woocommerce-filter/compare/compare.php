	...
	class KlbCompare {
		...
		function init() {
			...
			self::$fields = apply_filters( 'klbcp_fields', [
				'remove'             => '&nbsp;',
				'image'             => esc_html__( 'Image', 'clotya-core' ),
				'name'             	=> esc_html__( 'Name', 'clotya-core' ),
				'sku'               => esc_html__( 'SKU', 'clotya-core' ),
				'rating'            => esc_html__( 'Rating', 'clotya-core' ),
				'price'             => esc_html__( 'Price', 'clotya-core' ),
				'stock'             => esc_html__( 'Stock', 'clotya-core' ),
				'weight'            => esc_html__( 'Weight', 'clotya-core' ),
				'add_to_cart'       => esc_html__( 'Add to cart', 'clotya-core' ),
			] );
			...
			add_shortcode( 'klbcp_list', [ $this, 'clotya_compare_list' ] );

			// add button for clotya_compare_action - product box
			add_action( 'clotya_compare_action', [ $this, 'clotya_compare_button' ], 11 );
			
			// add button for single
			add_action( 'clotya_single_compare', [$this,'clotya_compare_button'], 40);
		}
		...
		function enqueue_scripts() {
			...
			wp_localize_script( 'klbcp-frontend', 'klbcp_vars', [
					...
					'view_page_text'     => esc_html__( 'View Compare Page', 'clotya-core' ),
					'close_text'     	 => esc_html__( 'Close', 'clotya-core' ),
					'message_added'      => esc_html__( '{name} has been added to Compare list.', 'clotya-core' ),
					'message_removed'    => esc_html__( '{name} has been removed from the Compare list.', 'clotya-core' ),
					'message_exists'     => esc_html__( '{name} is already in the Compare list.', 'clotya-core' ),
					'limit'              => 100,
					'limit_notice'       => esc_html__( 'You can add a maximum of {limit} products to the comparison table.', 'clotya-core' ),
					'button_text'        => apply_filters( 'klbcp_button_text', esc_html__( 'Compare', 'clotya-core' ) ),
					'button_text_added'  => apply_filters( 'klbcp_button_text_added', esc_html__( 'Compare', 'clotya-core' ) ),
				]
			);
		}
			...
			if ( is_array( $products ) && ( count( $products ) > 0 ) ) {
				...
				foreach ( $products as $product_id ) {
					...
					foreach ( $saved_fields as $saved_field ) {
						switch ( $saved_field ) {
							case 'remove':
								//remove button
								$products_data[ $product_id ]['remove'] = ' <span class="klbcp-remove" data-id="' . $product_id . '"><i class="klbth-icon-xmark"></i> ' . esc_html__( 'Remove', 'clotya-core' ) . '</span>';
				...
				$table = '<div class="klbcp-no-result">
							<div class="cart-empty-page">
								...
								<p class="cart-empty">'.esc_html__( 'The comparison table is empty.', 'clotya-core' ).'</p>
		...
		function clotya_compare_button( $attrs ) {
			...
			if ( $attr_id ) {
				// button text
				$text = esc_html__( 'Compare', 'clotya-core' );
		...
		function clotya_compare_list( $attrs ) {
		...
		function account_items( $items ) {
			...
			if ( ! isset( $items['compare'] ) ) {
				$items['compare'] = apply_filters( 'klbcp_myaccount_compare_label', esc_html__( 'Compare', 'clotya-core' ) );
			}
		...
		public static function get_page_url() {
			$page_id  = get_theme_mod('clotya_compare_page');
