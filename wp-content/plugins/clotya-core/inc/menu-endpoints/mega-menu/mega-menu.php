...
*************************************************/ 
function clotya_megamenu_admin_scripts() {
    wp_register_script('klb-mega-menu', plugins_url( 'js/admin-mega-menu.js', __FILE__ ), false, '1.0');
}
add_action( 'admin_enqueue_scripts', 'clotya_megamenu_admin_scripts' );

/*************************************************
## Fynode Mega Menu Custom Fields
*************************************************/
function clotya_mega_menu_custom_fields( $item_id, $item ) {
    ...
    <div class="et_menu_options">
        ...
        <div class="clotya-field-elementor-template description description-wide">
			<?php if( $item->classes[0] == 'klb-elementor-template'){ ?>
				<label for="menu_item_elementor-template-<?php echo esc_attr($item_id); ?>">
					<?php esc_html_e( 'Elementor Template', 'clotya-core'  ); ?><br />
					<select class="widefat code edit-menu-item-custom" id="menu_item_elementor_template-<?php echo esc_attr($item_id); ?>" name="menu_item_elementor_template[<?php echo esc_attr($item_id); ?>]">
						<?php foreach(clotya_get_elementorTemplates('section') as $key => $value){ ?>
				...
				<div class="clotya-field-elementor-template-width">
					<p>
						<label for="menu_item_elementor-template-width-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Sub Menu Width', 'clotya-core'  ); ?><br />
							...
							<select class="widefat code edit-menu-item-custom" id="menu_item_elementor_template_width-<?php echo esc_attr($item_id); ?>" name="menu_item_elementor_template_width[<?php echo esc_attr($item_id); ?>]">
								<option value="large" <?php echo esc_attr($menu_item_elementor_template_width == 'large' ? "selected='selected'" : ''); ?>><?php esc_html_e('Large', 'clotya-core'); ?></option>
								<option value="has-image" <?php echo esc_attr($menu_item_elementor_template_width == 'has-image' ? "selected='selected'" : ''); ?>><?php esc_html_e('Medium', 'clotya-core'); ?></option>
							</select>
						</label>
					</p>
				</div>
...
add_action( 'wp_nav_menu_item_custom_fields', 'clotya_mega_menu_custom_fields', 10, 2 );

/*************************************************
## Fynode Save menu item meta
*************************************************/
function clotya_mega_menu_nav_update( $menu_id, $menu_item_db_id) {
...
add_action( 'wp_update_nav_menu_item', 'clotya_mega_menu_nav_update', 10, 2 );

/*************************************************
## Fynode Nav Menu Mega Menu Endpoints Output
*************************************************/ 
function clotya_nav_menu_mega_menu_output( $itemOutput, $item ) {
	...
	if (! empty( $menu_item_iconfield ) && ! empty( $itemOutput ) && is_string( $itemOutput )&& strpos( $item->classes[0], 'klb-elementor-template' ) !== false) {
		...
		ob_start();
		do_action('clotya_submenu', $menu_item_iconfield);
...
if ( !is_admin() ) {
	add_filter( 'walker_nav_menu_start_el','clotya_nav_menu_mega_menu_output' , 50, 2 );
	add_filter( 'megamenu_walker_nav_menu_start_el', 'clotya_nav_menu_mega_menu_output', 50, 2 );
}

/*************************************************
## Add Action for clotya_submenu do_action 
*************************************************/
add_action( 'clotya_submenu', 'clotya_get_elementor_template', 10);

/*************************************************
## Add mega-menu class for parent if elementor template exist
*************************************************/
function clotya_megamenu_nav_objects( $sorted_menu_items, $args ) {
    $last_top = 0;
...
add_filter( 'wp_nav_menu_objects', 'clotya_megamenu_nav_objects', 10, 2 );
